...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        successors = []
        # x, y = state[0]
        # dx, dy = Actions.directionToVector(Directions.NORTH)
        # nextx, nexty = int(x + dx), int(y + dy)
        # print(x,y,dx,dy,nextx, nexty, sep="---")
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                corners = state[1][:]
                if newPosition in corners:
                    # remove khoi corners
                    temp = list(corners)
                    temp.remove(newPosition)
                    corners = tuple(temp)
                newState = (newPosition, corners)
                actionCost = 1
...
def cornersHeuristic(state, problem):
    ...
    while len(corners) > 0:
        nextCorner = nextPoint(current, corners)
        sum += util.manhattanDistance(current, nextCorner)
        current = nextCorner
        # xoá corner vừa đến khỏi list
        temp = list(corners)
